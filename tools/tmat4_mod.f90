...
module tmat4
  ...
  use cont_unstable
  use potential3
  use steuerung   			!  This we inserted because one needs the flag "readit" (label:maketables)
...
contains
  ...
  subroutine tmat(ecm,ivb,ive,k_j) ! Here, we introduced an "if" condition to fill the variable "tmatrix" with the interpolated values from the read-in table (label:maketables)
      ...
      if (readit==1) then
        write(0,*)'tmat 1'
        call qon_c(ecm,qon)  		! on-shell momentum for each channel. I copied this from below, because I think one needs it later somewhere.
        call tmat_gauss(ecm)
        call prop_cc(ecm)    ! propagator
        call restore_tnp(ecm,k_j)	! This is now the place where "tmatrix" is filled with the interpolated values from the table.
        ...
        !    write(0,*)'test qon'                          !  test1
!            write(0,*)qon(14),qon(16)!,qon(18),qon(19)     !  test1
        call tmat_gauss(ecm)
        !    write(0,*)u(15,n1),u(16,n1),u(18,n1),u(19,n1)    !  test2
        call vmat(ecm,ivb,ive,k_j)  ! potential (non-pole part)
        !    write(0,*)vmatrix(1,14,16,1,1),vmatrix(1,14,19,1,1),vmatrix(1,16,16,1,1),vmatrix(1,16,19,1,1)
        call tmat_non_pole(ecm,ivb,ive,k_j)
!            write(0,*)tmatrix(n1,n1,1,14,14),tmatrix(n1,n1,1,16,16)
      end if
  end subroutine tmat
...
!        write(0,*)'Do rsheet==2 part'
        ...
        thres(14) =      ms_b_c(14)+ms_f_c(14)	!   mD+mLc
        thres(15) =      ms_b_c(15)+ms_f_c(15)	!   mD+mSigmac
        thres(16) =      ms_b_c(16)+ms_f_c(16)	!   mDs+mLc
        thres(17) = thres(16)
        thres(18) = thres(16)
        thres(19) =      ms_b_c(19)+ms_f_c(19)	!   mDs+mSc
        thres(20) = thres(19)
        thres(21) = thres(19)
        ...
        if (dble(ecm).gt.(thres(14))) then
          u(14,n1) = +(0.d0,1.d0)*tpi*qon(14) &	! DLc channel 2nd sheet
              & *sqrt(qon(14)**2+ms_b_c(14)**2)*sqrt(qon(14)**2+ms_f_c(14)**2)/ecm
        end if
        if (dble(ecm).gt.thres(15)) then
          u(15,n1) = +(0.d0,1.d0)*tpi*qon(15) &	! DSc channel 2nd sheet				SHOULD GT, DONT FORGET TO CHANGE BACK!!!!
              & *sqrt(qon(15)**2+ms_b_c(15)**2)*sqrt(qon(15)**2+ms_f_c(15)**2)/ecm
        end if
        if (dble(ecm).gt.(thres(16))) then
          u(16,n1) = +(0.d0,1.d0)*tpi*qon(16) &	! DsLc channel 2nd sheet
              & *sqrt(qon(16)**2+ms_b_c(16)**2)*sqrt(qon(16)**2+ms_f_c(16)**2)/ecm
          u(17,n1) = +(0.d0,1.d0)*tpi*qon(16) &	! DsLc channel 2nd sheet
              & *sqrt(qon(16)**2+ms_b_c(16)**2)*sqrt(qon(16)**2+ms_f_c(16)**2)/ecm
          u(18,n1) = +(0.d0,1.d0)*tpi*qon(16) &	! DsLc channel 2nd sheet
              & *sqrt(qon(16)**2+ms_b_c(16)**2)*sqrt(qon(16)**2+ms_f_c(16)**2)/ecm
        end if
        if (dble(ecm).gt.thres(19)) then
          u(19,n1) = +(0.d0,1.d0)*tpi*qon(19) &	! DsSc channel 2nd sheet				SHOULD GT, DONT FORGET TO CHANGE BACK!!!!
              & *sqrt(qon(19)**2+ms_b_c(19)**2)*sqrt(qon(19)**2+ms_f_c(19)**2)/ecm
          u(20,n1) = +(0.d0,1.d0)*tpi*qon(19) &	! DsSc channel 2nd sheet				SHOULD GT, DONT FORGET TO CHANGE BACK!!!!
              & *sqrt(qon(19)**2+ms_b_c(19)**2)*sqrt(qon(19)**2+ms_f_c(19)**2)/ecm
          u(21,n1) = +(0.d0,1.d0)*tpi*qon(19) &	! DsSc channel 2nd sheet				SHOULD GT, DONT FORGET TO CHANGE BACK!!!!
              & *sqrt(qon(19)**2+ms_b_c(19)**2)*sqrt(qon(19)**2+ms_f_c(19)**2)/ecm
        end if
  ...
  subroutine prop_cc(ecm)
      ...
      do i=1,n
        ...
        e1 = cdsqrt(xg2+ms_f_c(14)**2)
        e2 = cdsqrt(xg2+ms_b_c(14)**2)
        u(14,i) =wgaus(i)*xg2/(ecm-e1-e2+epsilo)    ! LambdacD prop

        e1 = cdsqrt(xg2+ms_f_c(15)**2)
        e2 = cdsqrt(xg2+ms_b_c(15)**2)
        u(15,i) =wgaus(i)*xg2/(ecm-e1-e2+epsilo)    ! SigmacD prop

        e1 = cdsqrt(xg2+ms_f_c(16)**2)
        e2 = cdsqrt(xg2+ms_b_c(16)**2)
        u(16,i) =wgaus(i)*xg2/(ecm-e1-e2+epsilo)    ! LambdacDs prop
        u(17,i) = u(16,i)
        u(18,i) = u(16,i)

        e1 = cdsqrt(xg2+ms_f_c(19)**2)
        e2 = cdsqrt(xg2+ms_b_c(19)**2)
        u(19,i) =wgaus(i)*xg2/(ecm-e1-e2+epsilo)    ! SigmacDs prop
        u(20,i) = u(19,i)
        u(21,i) = u(19,i)

        ! Switch off KY and DsYc propagators:
  ...
  subroutine vmat(ecm,ivb,ive,k_j)
      ...
      do ix=1,n1
        do iy=ix,n1
          ...
          xmev=xgau(14,ix)
          ymev=xgau(14,iy)
          call ccpot(v,ymev,xmev,ecm,9,9)
          do iv=ivb,ive
            vmatrix(iv,14,14,ix,iy)=v(iv)    ! LamcD->LamcD
            vmatrix(iv,14,14,iy,ix)=v(iv)
          end do
          xmev=xgau(15,ix)
          ymev=xgau(15,iy)
          call ccpot(v,ymev,xmev,ecm,10,10)
          do iv=ivb,ive
            vmatrix(iv,15,15,ix,iy)=v(iv)    ! SigcD->SigcD
            vmatrix(iv,15,15,iy,ix)=v(iv)
          end do
          xmev=xgau(16,ix)
          ymev=xgau(16,iy)
          call ccpot(v,ymev,xmev,ecm,11,11)
          do iv=ivb,ive
            vmatrix(iv,16,16,ix,iy)=v(iv)    ! LamcDs->LamcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,16,16,iy,ix)=v(iv)
            vmatrix(iv,17,17,ix,iy)=v(iv+12) ! <j-+1/2,3/2| |j-+1/2,3/2>
            vmatrix(iv,17,17,iy,ix)=v(iv+12)
            vmatrix(iv,18,18,ix,iy)=v(iv+20) ! <j+-3/2,3/2| |j+-3/2,3/2>
            vmatrix(iv,18,18,iy,ix)=v(iv+20)

            vmatrix(iv,17,16,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,17,16,iy,ix)=v(iv+24)
            vmatrix(iv,16,17,ix,iy)=v(iv+24)
            vmatrix(iv,16,17,iy,ix)=v(iv+4)

            vmatrix(iv,18,16,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,18,16,iy,ix)=v(iv+28)
            vmatrix(iv,16,18,ix,iy)=v(iv+28)
            vmatrix(iv,16,18,iy,ix)=v(iv+8)

            vmatrix(iv,18,17,ix,iy)=v(iv+16) ! <j+-3/2,3/2| |j-+1/2,3/2>
            vmatrix(iv,18,17,iy,ix)=v(iv+32)
            vmatrix(iv,17,18,ix,iy)=v(iv+32)
            vmatrix(iv,17,18,iy,ix)=v(iv+16)
          end do
          xmev=xgau(19,ix)
          ymev=xgau(19,iy)
          call ccpot(v,ymev,xmev,ecm,12,12)
          do iv=ivb,ive
            vmatrix(iv,19,19,ix,iy)=v(iv)    ! SigcDs->SigcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,19,19,iy,ix)=v(iv)
            vmatrix(iv,20,20,ix,iy)=v(iv+12) ! <j-+1/2,3/2| |j-+1/2,3/2>
            vmatrix(iv,20,20,iy,ix)=v(iv+12)
            vmatrix(iv,21,21,ix,iy)=v(iv+20) ! <j+-3/2,3/2| |j+-3/2,3/2>
            vmatrix(iv,21,21,iy,ix)=v(iv+20)

            vmatrix(iv,20,19,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,20,19,iy,ix)=v(iv+24)
            vmatrix(iv,19,20,ix,iy)=v(iv+24)
            vmatrix(iv,19,20,iy,ix)=v(iv+4)

            vmatrix(iv,21,19,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,21,19,iy,ix)=v(iv+28)
            vmatrix(iv,19,21,ix,iy)=v(iv+28)
            vmatrix(iv,19,21,iy,ix)=v(iv+8)

            vmatrix(iv,21,20,ix,iy)=v(iv+16) ! <j+-3/2,3/2| |j-+1/2,3/2>
            vmatrix(iv,21,20,iy,ix)=v(iv+32)
            vmatrix(iv,20,21,ix,iy)=v(iv+32)
            vmatrix(iv,20,21,iy,ix)=v(iv+16)
          end do
        end do
        ...
        do iy=1,n1
          ...
          xmev=xgau(15,ix)   ! Sigmac D
          ymev=ygau(14,iy)   ! Lambdac D
          call ccpot(v,ymev,xmev,ecm,9,10)
          do iv=ivb,ive
            vmatrix(iv,15,14,ix,iy)=v(iv)  ! LamcD->SigcD
            vmatrix(iv,14,15,iy,ix)=v(iv)
          end do
          xmev=xgau(2,ix)   ! N rho
          ymev=ygau(12,iy)  ! Lambda K
          ...
          !!new for Dstar case
          xmev=xgau(16,ix)   ! Lambdac Ds
          ymev=ygau(14,iy)   ! Lambdac D
          call ccpot(v,ymev,xmev,ecm,9,11)
          do iv=ivb,ive
            vmatrix(iv,16,14,ix,iy)=v(iv)  ! LamcD->LamcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,14,16,iy,ix)=-v(iv)
            vmatrix(iv,17,14,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,14,17,iy,ix)=-v(iv+4)
            vmatrix(iv,18,14,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,14,18,iy,ix)=-v(iv+8)
          end do
          xmev=xgau(19,ix)   ! Sigmac Ds
          ymev=ygau(14,iy)   ! Lambdac D
          call ccpot(v,ymev,xmev,ecm,9,12)
          do iv=ivb,ive
            vmatrix(iv,19,14,ix,iy)=v(iv)  ! LamcD->SigcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,14,19,iy,ix)=-v(iv)
            vmatrix(iv,20,14,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,14,20,iy,ix)=-v(iv+4)
            vmatrix(iv,21,14,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,14,21,iy,ix)=-v(iv+8)
          end do
          xmev=xgau(16,ix)   ! Lambdac Ds
          ymev=ygau(15,iy)   ! Sigmac D
          call ccpot(v,ymev,xmev,ecm,10,11)
          do iv=ivb,ive
            vmatrix(iv,16,15,ix,iy)=v(iv)  ! SigcD->LamcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,15,16,iy,ix)=-v(iv)
            vmatrix(iv,17,15,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,15,17,iy,ix)=-v(iv+4)
            vmatrix(iv,18,15,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,15,18,iy,ix)=-v(iv+8)
          end do
          xmev=xgau(19,ix)   ! Sigmac Ds
          ymev=ygau(15,iy)   ! Sigmac D
          call ccpot(v,ymev,xmev,ecm,10,12)
          do iv=ivb,ive
            vmatrix(iv,19,15,ix,iy)=v(iv)  ! SigcD->SigcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,15,19,iy,ix)=-v(iv)
            vmatrix(iv,20,15,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,15,20,iy,ix)=-v(iv+4)
            vmatrix(iv,21,15,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,15,21,iy,ix)=-v(iv+8)
          end do
          xmev=xgau(19,ix)   ! Sigmac Ds
          ymev=ygau(16,iy)   ! Lambdac Ds
          call ccpot(v,ymev,xmev,ecm,11,12)
          do iv=ivb,ive
            vmatrix(iv,19,16,ix,iy)=v(iv)  ! LamcDs->SigcDs <j-+1/2,1/2| |j-+1/2,1/2>
            vmatrix(iv,16,19,iy,ix)=v(iv)
            vmatrix(iv,20,16,ix,iy)=v(iv+4)  ! <j-+1/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,16,20,iy,ix)=v(iv+4)
            vmatrix(iv,21,16,ix,iy)=v(iv+8)  ! <j+-3/2,3/2| |j-+1/2,1/2>
            vmatrix(iv,16,21,iy,ix)=v(iv+8)

            vmatrix(iv,19,17,ix,iy)=v(iv+24)  ! <j-+1/2,1/2| |j-+1/2,3/2>
            vmatrix(iv,17,19,iy,ix)=v(iv+24)
            vmatrix(iv,20,17,ix,iy)=v(iv+12)  ! <j-+1/2,3/2| |j-+1/2,3/2>
            vmatrix(iv,17,20,iy,ix)=v(iv+12)
            vmatrix(iv,21,17,ix,iy)=v(iv+16)  ! <j+-3/2,3/2| |j-+1/2,3/2>
            vmatrix(iv,17,21,iy,ix)=v(iv+16)

            vmatrix(iv,19,18,ix,iy)=v(iv+28)  ! <j-+1/2,1/2| |j+-3/2,3/2>
            vmatrix(iv,18,19,iy,ix)=v(iv+28)
            vmatrix(iv,20,18,ix,iy)=v(iv+32)  ! <j-+1/2,3/2| |j+-3/2,3/2>
            vmatrix(iv,18,20,iy,ix)=v(iv+32)
            vmatrix(iv,21,18,ix,iy)=v(iv+20)  ! <j+-3/2,3/2| |j+-3/2,3/2>
            vmatrix(iv,18,21,iy,ix)=v(iv+20)
          end do
