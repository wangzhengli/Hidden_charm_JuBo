subroutine make_table		!(label:maketables)
  ...
  integer :: table_bgd_unit(1:4),table_short_unit(1:4)  ! unit to write or read the table, overwritten if k_j changes!
  complex(kind(0.d0)) :: ez,phasfacnew
  ...
  do k_j=k_b,k_e  		! loop over angular momentum
    do iv=1,4 			! Open storage to write table PW by PW. Note: this must be done inside the k_j loop!
      ...
      table_short_unit(iv)=4030+iv
      open(table_short_unit(iv),file='t_short_tables_'//bgd(iv,k_j))
    end do  ! iv=1,4
    if (writeit==1.and.readit==0) then		! Wheteher or not to calculate T^NP and write it
...
!          write(table_short_unit(iv),156)dble(ez),tmatrix(n1,n1,iv,14,14)*phasfacnew(ez,14,14), &
 !           & tmatrix(n1,n1,iv,14,15)*phasfacnew(ez,14,15), &
  !          & tmatrix(n1,n1,iv,15,15)*phasfacnew(ez,15,15), &
   !         & tmatrix(n1,n1,iv,14,16)*phasfacnew(ez,14,16), &
    !        & tmatrix(n1,n1,iv,16,16)*phasfacnew(ez,16,16), &
     !       & tmatrix(n1,n1,iv,17,17)*phasfacnew(ez,17,17), &
      !      & tmatrix(n1,n1,iv,14,19)*phasfacnew(ez,14,19), &
       !     & tmatrix(n1,n1,iv,16,19)*phasfacnew(ez,16,19), &
        !    & tmatrix(n1,n1,iv,17,19)*phasfacnew(ez,17,19), &
         !   & tmatrix(n1,n1,iv,17,20)*phasfacnew(ez,17,20), &
          !  & tmatrix(n1,n1,iv,19,19)*phasfacnew(ez,19,19)
          !write(0,*)'test maketable'			!test
          !write(0,*)tmatrix(nj,ni,iv,ic,jc),tmatrix(1,1,1,16,16)      !test
          !write(0,156)tmatrix(nj,ni,iv,ic,jc)     !test
        end do
